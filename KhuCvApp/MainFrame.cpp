...
EVT_MENU(IDM_CONTEXT_SET_QUAD, CMainFrame::OnSetQuad)
EVT_MENU(IDM_CONTEXT_SET_LABEL, CMainFrame::OnSetLabel)
EVT_MENU(IDM_CONTEXT_SAVE_NEXT, CMainFrame::OnSaveNext)
EVT_MENU(IDM_CONTEXT_DELETE_QUAD, CMainFrame::OnDeleteQuad)
END_EVENT_TABLE()
...
#endif
{
    ...
    const int nAcce = 9;
    wxAcceleratorEntry entries[nAcce];
    ...
    entries[5].Set(wxACCEL_CTRL, (int)'Q', IDM_CONTEXT_SET_QUAD);
    entries[6].Set(wxACCEL_CTRL, (int)'L', IDM_CONTEXT_SET_LABEL);
    entries[7].Set(wxACCEL_CTRL, (int)'A', IDM_CONTEXT_SAVE_NEXT);
    entries[8].Set(wxACCEL_CTRL, (int)'W', IDM_CONTEXT_DELETE_QUAD);
    

    wxAcceleratorTable accel(nAcce, entries);
...
void CMainFrame::OnSetQuad(wxCommandEvent& event) {
#ifndef _KHUCV_SDI
    CChildFrame* pChild = (CChildFrame*)GetActiveChild();
    if (pChild) {
        pChild->m_pClientView->ProcessEvent(event);
...
void CMainFrame::OnSetLabel(wxCommandEvent& event) {
#ifndef _KHUCV_SDI
    CChildFrame* pChild = (CChildFrame*)GetActiveChild();
    if (pChild) {
        pChild->m_pClientView->ProcessEvent(event);
...
void CMainFrame::OnSaveNext(wxCommandEvent& event) {
#ifndef _KHUCV_SDI
    CChildFrame* pChild = (CChildFrame*)GetActiveChild();
    if (pChild) {
        pChild->m_pClientView->ProcessEvent(event);
...
void CMainFrame::OnDeleteQuad(wxCommandEvent& event) {
#ifndef _KHUCV_SDI
    CChildFrame* pChild = (CChildFrame*)GetActiveChild();
    if (pChild) {
        pChild->m_pClientView->ProcessEvent(event);
