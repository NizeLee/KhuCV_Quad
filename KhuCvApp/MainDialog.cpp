...
EVT_BUTTON(IDC_NEXT, CMainDialog::OnNext)
EVT_BUTTON(IDC_SAVE, CMainDialog::OnSave)
EVT_BUTTON(IDC_SAVE_NEXT, CMainDialog::OnSaveNext)
EVT_BUTTON(IDC_SAVE_NEXT_COPY, CMainDialog::OnSaveNextCopy)
EVT_BUTTON(IDC_EXAMPLE, CMainDialog::OnExample)
EVT_TIMER(-1, CMainDialog::OnTimer)
EVT_COMBOBOX_CLOSEUP(IDC_SEL_CAM, CMainDialog::OnCloseup)
EVT_COMBOBOX_CLOSEUP(IDC_SEL_ZOOM, CMainDialog::OnCloseupSelZoom)
END_EVENT_TABLE()
...
//	m_pVideoFileCheck = new wxCheckBox(this, IDC_VIDEO_FILE_CHECK, wxT("Video file"), wxDefaultPosition, wxSize(90, 20));
	...
	m_pHbox[0]->Add(m_pDisplaySrcPathText, 0, wxLEFT, 5);
...
//	m_pHbox[1]->Add(m_pSaveFrameCheck, 0, wxLEFT, 5);
	m_pHbox[1]->Add(m_pDisplayDesPathText, 1);
	...
	long indx2 = m_pListCtrl->InsertColumn(1, "File path", wxLIST_FORMAT_LEFT, 270);
	long indx3 = m_pListCtrl->InsertColumn(2, "File name", wxLIST_FORMAT_LEFT, 270);
	...
	m_pCopyCheck = new wxCheckBox(this, IDC_COPY_CHECK, wxT("Copy"), wxDefaultPosition, wxSize(70, 20));

	wxArrayString ZoomString;
	...
	m_pSelZoom = new wxComboBox(this, IDC_SEL_ZOOM, wxEmptyString, wxDefaultPosition, wxSize(120, 20), ZoomString, wxCB_DROPDOWN | wxCB_READONLY);
	m_pSelZoom->SetSelection(0);

	m_pHbox[3]->Add(m_pStartNum, 1);
	m_pHbox[3]->Add(m_pEndNum, 0, wxLEFT, 5);
	m_pHbox[3]->Add(m_pCopyCheck, 0, wxLEFT, 5);
	m_pHbox[3]->Add(m_pSelZoom, 0, wxLEFT, 5);
	...
	for (int i = 0; i < g_classNum; ++i) {
		CamString.Add(g_className[i]);
	}

	m_pSelCam = new wxComboBox(this, IDC_SEL_CAM, wxEmptyString, wxDefaultPosition, wxSize(120, 20), CamString, wxCB_DROPDOWN | wxCB_READONLY);
	m_pSelCam->SetSelection(0);
	m_pRunButton = new wxButton(this, IDC_RUN, wxT("Run"), wxDefaultPosition, wxSize(50, 20));
	m_pSaveNextButton = new wxButton(this, IDC_SAVE_NEXT, wxT("Save Next"), wxDefaultPosition, wxSize(100, 20));
	m_pSaveButton = new wxButton(this, IDC_SAVE, wxT("Save"), wxDefaultPosition, wxSize(50, 20));
	m_pNextButton = new wxButton(this, IDC_NEXT, wxT("Next"), wxDefaultPosition, wxSize(50, 20));
...
//	m_pHbox[4]->Add(m_pRunButton, 1);
	m_pHbox[4]->Add(m_pSaveNextButton, 0, wxLEFT, 5);
	m_pHbox[4]->Add(m_pSaveButton, 0, wxLEFT, 5);
	m_pHbox[4]->Add(m_pNextButton, 0, wxLEFT, 5);
//	m_pHbox[4]->Add(m_pPauseButton, 1, wxLEFT, 5);
//	m_pHbox[4]->Add(m_pStepCheck, 1, wxLEFT, 5);
//	m_pHbox[4]->Add(m_pVerboseCheck, 0, wxLEFT, 5);

	
	m_pViewRectCheck = new wxCheckBox(this, IDC_VIEW_RECT, wxT("View rect"), wxDefaultPosition, wxSize(120, 20));
	m_pViewQuadCheck = new wxCheckBox(this, IDC_VIEW_QUAD, wxT("View quad"), wxDefaultPosition, wxSize(120, 20));
	m_pViewRectCheck->SetValue(true);
	m_pViewQuadCheck->SetValue(true);
	m_pHbox[5]->Add(m_pViewRectCheck, 1);
	m_pHbox[5]->Add(m_pViewQuadCheck, 0, wxLEFT, 5);
//	m_pHbox[5]->Add(m_pDisplayDesPathText, 0);
...
void CMainDialog::OnCloseupSelZoom(wxCommandEvent& event) {
	CClientView* pView = GetActiveClientView();
	pView->m_Scale = m_pSelZoom->GetSelection()*50 + 100;
}

void CMainDialog::OnCloseup(wxCommandEvent& event) {
	CClientView* pView = GetActiveClientView();
	pView->m_nCurrentClassIndex = m_pSelCam->GetSelection();
}
...
void CMainDialog::OnSelSrcFileFolder(wxCommandEvent& event) {
	...
	*m_pStartNum << 0;
	...
	bool bFileMode = false;// m_pVideoFileCheck->GetValue();
	...
	if (!bFileMode) {
		...
		CClientView* pView = GetActiveClientView();
		pView->m_LabelPath = dirDialog.GetPath() + "/../TextLabel";

		if (!wxDirExists(dirDialog.GetPath() + "/../TextLabel")) {
			if (!wxMkdir(dirDialog.GetPath() + "/../TextLabel"))
				wxMessageBox("Label directory was not made.");
		}
		...
		while (cont) {
			...
			m_pListCtrl->SetItem(index, 2, filename);
	...
	*m_pStartNum << nSel;
	...
	if (!bFileMode) {
		...
		if (m_bRunTimer) {
			...
			m_nProcessingNum = nSel;

			CClientView* pView = GetActiveClientView();

			if (!m_pCopyCheck->GetValue()) {
				pView->ClearLabels();
			}
...
void CMainDialog::OnTimer(wxTimerEvent& event) {
	...
	if (event.GetId() == ID_TIMER_CAM_RUN) {
		...
		if (false) { //m_pSaveFrameCheck->GetValue()) {
	...
	if(bLoaded) {
		...
		if (!CurrentImage.empty()) {
			...
			wxString str = m_pListCtrl->GetItemText(m_nProcessingNum, 2);

			wxString filepath1 = pView->m_LabelPath + "/" + str, filepath;
			int pos = filepath1.rfind(".");
			filepath = filepath1.substr(0, pos);
			wxString labelPath = filepath + ".txt";

			if (wxFileExists(labelPath)) {
				pView->ClearLabels();
				pView->ReadLabel(str);
			}
			else {
			...
			CMainFrame* pMainFrame = (CMainFrame*)GetParent();
			CChildFrame* pChildFrame = (CChildFrame*)(pMainFrame->GetActiveChild());
			pChildFrame->ClearAllImages();
#else
			CMainFrame* pMainFrame = (CMainFrame*)GetParent();
			pMainFrame->ClearAllImages();
#endif
		...
		// Project Run;
		m_Project.Run(CurrentImage, Output, m_bFirstRun, false);// m_pVerboseCheck->GetValue());
	...
	if (true) m_bRunPause = true;
}

void CMainDialog::OnSave(wxCommandEvent& event) {
	//m_bRunPause = !m_bRunPause;

	CClientView* pView = GetActiveClientView();
	wxString str = m_pListCtrl->GetItemText(m_nProcessingNum - 1, 2);

	if (m_bRunTimer)
		pView->SaveLabel(str);
}

void CMainDialog::OnNext(wxCommandEvent& event) {
	CClientView* pView = GetActiveClientView();
...
void CMainDialog::OnSaveNext(wxCommandEvent& event) {
	CClientView* pView = GetActiveClientView();
	wxString str = m_pListCtrl->GetItemText(m_nProcessingNum - 1, 2);
...
void CMainDialog::OnSaveNextCopy(wxCommandEvent& event) {
	CClientView* pView = GetActiveClientView();
	wxString str = m_pListCtrl->GetItemText(m_nProcessingNum - 1, 2);
...
void CMainDialog::OnRun(wxCommandEvent& event) {
	bool bVidieFileMode = false;// m_pVideoFileCheck->GetValue();
	bool bCamMode;

	//int nCamNum = m_pSelCam->GetSelection()-1;
	bCamMode = false;// nCamNum >= 0 ? true : false;
